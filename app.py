...
def api_query_stream():
    ...
    try:
        ...
        # FIKSET: Bruk SAMME session ID for både frontend og streaming
        ...
        # Get conversation memory using same session ID
        conversation_memory = get_conversation_memory(session_id)
        app.logger.info(f"🧠 STREAM API: Using session ID for memory: {session_id}")
                ...
                try:
                    ...
                    if should_save_memory:
                        try:
                            ...
                            update_conversation_memory(session_id, query_req.question, result.get('answer', ''))
                            app.logger.info(f"✅ ASYNC: Updated conversation memory for session {session_id}")
                            
                            # Save to database using session_manager
                            from src.session_manager import session_manager
                            
                            # Get conversation ID from request data if available
        ...
        # FIKSET: Return samme session_id for både frontend og streaming
        response_data = {
            'session_id': session_id,           # Same ID for consistency
        ...
        app.logger.info(f"🔑 STREAM API: Using unified session_id: {session_id}")
        return jsonify(response_data)
